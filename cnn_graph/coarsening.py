    ...
    if len(xs) == 2:
        ...
        for i,j in enumerate(indices):
            if j < M:
                ...
                xnew[:,i,:] = np.squeeze(np.zeros((N,1,K)))
        return xnew
        
def func_test(x, indices):
    """ Test function added by Pierre Besson
    This is to test exactly how to handle 3D arrays"""
