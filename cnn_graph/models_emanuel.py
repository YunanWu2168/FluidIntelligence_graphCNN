...
#         shutil.rmtree(self._get_path('summaries'), ignore_errors=False)          # removed by Emanuel

        writer = tf.summary.FileWriter(self._get_path('summaries'), self.graph)
#         shutil.rmtree(self._get_path('checkpoints'), ignore_errors=False)        # removed by Emanuel
        
#         os.makedirs(self._get_path('checkpoints'))                               # removed by Emanuel
...
#         return os.path.join(path, '..', folder, self.dir_name)
    ...
    def _weight_variable(self, shape, regularization=True):
        ### Added by Emanuel ###
        initial = tf.initializers.he_uniform(seed=42)
        ###
    ...
    def _bias_variable(self, shape, regularization=True):
        ### Added by Emanuel ###
        initial = tf.initializers.he_uniform(seed=7)
        ###
...
class cgcnn(base_model):
    ...
    def __init__(self, L, F, K, p, M, filter='chebyshev5', brelu='b1relu', pool='mpool1',
        ...
        #### Added by Emanuel: V at the end of the line
    ...
    def fc(self, x, Mout, relu=True):
        ...
        return tf.nn.leaky_relu(x) if relu else x
