...
#         return x_fc
    def _inference(self, x, do_cam, is_training):
        ...
        with tf.variable_scope('fc_layer'):
            ...
            W = self._weight_variable([int(Min), self.M[-1]], regularization=True)
            x_fc = tf.matmul(gap, W)
            print(gap.shape, W.shape)
            print(x.shape,self.M[-1])
            # cam = tf.multiply(x, W[:,0])
            # cam = tf.reduce_sum(cam, 2)
            # #Test1, Test2 = cam.get_shape()
            # #print('cam = {0} x {1}'.format(Test1, Test2))
            # i = 1
            # while i < self.M[-1]:
            #     tt = tf.multiply(x, W[:,i])
            #     tt = tf.reduce_sum(tt, 2)
            #     cam = tf.stack([cam, tt], 2)
            #     i += 1
            #### Added by Yunan
