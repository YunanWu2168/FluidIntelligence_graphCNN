...
# --------------------- BASE MODEL CLASS FOR GRAPH CONVOLUTION MODEL ---------------------
...
class base_model(object):
    ...
    def fit(self, train_data, train_labels, val_data, val_labels):
        ...
        for step in range(1, num_steps+1):
            ...
            ## Added by Pierre for debugging purposes
            # print('Class distribution in batch: ', np.unique(batch_labels, return_counts=True)[1])
            ##
            ...
            if step % self.eval_frequency == 0 or step == num_steps:
                ...
                #accuracies.append(accuracy)
                #losses.append(loss)
                ## Added by Pierre to track perf on training data
                # Print confusion matrix
                #string, accuracy_train, f1_train, loss_train = self.evaluate(batch_data, batch_labels, sess)
                #print('  training   {}'.format(string))
                #print('Nb examples per class : 0 = ' + str(np.sum(batch_labels == 0)) + ' ; 1 = ' + str(np.sum(batch_labels == 1)))
                #print(' Expected training classes : 0 = {} , 1 = {}'.format(np.sum(batch_labels == 0), np.sum(batch_labels == 1))
                ##
    ...
    def loss(self, logits, labels, regularization):
        ...
        with tf.name_scope('loss'):
            ...
            tf.summary.scalar('loss/tr_accuracy', tr_accuracy)
    ...
    def training(self, loss, learning_rate, decay_steps, decay_rate=0.95, momentum=0.9):
        ...
        with tf.name_scope('training'):
            ...
            # grads = [(tf.clip_by_value(grad, -1.0, 1.0), var) for grad, var in grads]
            ##
...
# --------------------- GRAPH CONVOLUTION MODEL (ONLY CHEBYSHEV FOR NOW) ---------------------
...
class cgcnn(base_model):
    ...
    def fc(self, x, Mout, relu=True):
        ...
        ## If logit layer, discard the bias for CAM
        if relu:
        ...
        ## Added by Pierre
        ## Set activation function for FC layers to leaky relu
        # return tf.nn.tanh(x) if relu else x
        ##
        #return tf.nn.relu(x) if relu else x
    ...
    def _inference(self, x, dropout):
        ...
        with tf.variable_scope('pre_conv'):
            with tf.name_scope('conv_block'):
                # x = self.batch_normalization_layer(x, self.V)
                # x = self.conv_bn_relu_layer(x, self.L[0], self.F[0], self.K[0])
                x = self.filter(x, self.L[0], self.F[0], self.K[0])
                # Add biases
                _,B,_ = x.get_shape()
                b = self._bias_variable([int(1), int(B), int(self.F[0])], regularization=False)
                x = x + b
        ...
        for i in range(len(self.p)):
            ...
            with tf.variable_scope('conv{}'.format(i+1)):
                with tf.name_scope('res_block'):
                    x1 = self.batch_normalization_layer(x, self.F[i])
                ...
                #with tf.variable_scope('conv_block_2'):
                    #x1 = self.batch_normalization_layer(x1, self.F[i])
                    #x1 = tf.nn.relu(x1)
                    #x1 = self.filter(x1, self.L[i], self.F[i], self.K[i])
        ...
        with tf.variable_scope('post_block'):
            with tf.name_scope('norm_relu_globpool'):
                x = self.batch_normalization_layer(x, F)
                x = tf.nn.relu(x)
                x = tf.reduce_mean(x, 1) # Global pool -> N x F
    ...
    def batch_normalization_layer(self, x, dimension):
        '''
        ...
        bn_layer = tf.nn.batch_normalization(x, mean, variance, beta, gamma, BN_EPSILON)
        tf.summary.histogram(beta.op.name, beta)
        tf.summary.histogram(gamma.op.name, gamma)
        
        return bn_layer
    
    
    def conv_bn_relu_layer(self, x, L, Fout, K):
        
        N,M,Fin = x.get_shape()
        
        x = self.filter(x, L, Fout, K)
        x = self.batch_normalization_layer(x, Fout)
        x = tf.nn.relu(x)
        #x = self.brelu(x)
        
        return x
    
    def bn_relu_conv_layer(self, x, L, Fout, K):
        
        N,M,Fin = x.get_shape()
        
        x = self.batch_normalization_layer(x, Fin)
        x = tf.nn.relu(x)
        #x = self.brelu(x)
    ...
    def res_block(self, x, L, Fout, K, p, is_first_block):
        N, M, Fin = x.get_shape()
        
        with tf.variable_scope('conv1_in_block'):
            if is_first_block:
                # conv1 = self.filter(x, L, Fout, K)
                conv1 = self.apool1(x, p)
            else:
                conv1 = self.batch_normalization_layer(x, Fin)
                #conv1 = self.brelu(conv1)
                conv1 = tf.nn.relu(conv1)
                conv1 = self.apool1(conv1, p)
                
        with tf.variable_scope('conv2_in_block'):
            conv2 = self.bn_relu_conv_layer(x, L, Fout, K)
            
        conv2 = self.pool(conv2, p)
